...
def augment_improved(x):
    # choose random
    augmentations = [random_saturation, tfimg.flip_up_down, tfimg.flip_left_right, tfimg.transpose,
                     random_contrast, random_brightness, identity]
    random_augment = np.random.choice(augmentations)
    x = random_augment(x)
    # random pad and crop
    x = tf.pad(x, paddings=[(0, 0), (4, 4), (4, 4), (0, 0)], mode='REFLECT')
    x = tf.map_fn(lambda batch: tf.image.random_crop(batch, size=(32, 32, 3)), x, parallel_iterations=cpu_count())
...
def random_contrast(x):
    return tfimg.random_contrast(x, 0.2, 0.5)


def random_saturation(x):
    return tfimg.random_saturation(x, 0.2, 0.5)


def random_brightness(x):
    return tfimg.random_brightness(x, 0.2)
...
def mixmatch(model, x, y, u, T, K, beta, model_name):
    batch_size = x.shape[0]
    if model_name == 'improved_mix_match':
        x_aug = augment_improved(x)
        u_aug = [None for _ in range(K)]
        for k in range(K):
            u_aug[k] = augment_improved(u)
    else:
        x_aug = augment(x)
        u_aug = [None for _ in range(K)]
        for k in range(K):
