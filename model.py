...
class WideResNet(tf.keras.Model):
    def __init__(self,  model_name, num_classes, depth=28, width=2, droprate=0., input_shape=(None, 32, 32, 3), **kwargs):
        ...
        if model_name == 'improved_mix_match':
            self.dense = tf.keras.layers.Dense(num_classes, kernel_regularizer=tf.keras.regularizers.l2(0.0001))
        else:
